...
def do_part_download(args):
    ...
    resp = conn.make_request("GET", bucket=bucket_name,
            key=key_name, headers={'Range':"bytes=%d-%d" % (min_byte, max_byte), 'x-amz-request-payer': 'requester'})
...
def main(src, dest, num_processes=2, split=32, force=False, verbose=False, quiet=False, secure=True, max_tries=5):
    ...
    bucket = s3.lookup(split_rs.netloc, headers={'x-amz-request-payer': 'requester'})
    if bucket == None:
        raise ValueError("'%s' is not a valid bucket" % split_rs.netloc)
    key = bucket.get_key(split_rs.path, headers={'x-amz-request-payer': 'requester'})
    if key is None:
      raise ValueError("'%s' does not exist." % split_rs.path)

    # Determine the total size and calculate byte ranges
    resp = s3.make_request("HEAD", bucket=bucket, key=key, headers={'x-amz-request-payer': 'requester'})
    if resp is None:
